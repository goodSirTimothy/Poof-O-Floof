...
public class PetFinderConnectionUtil {
  ...
  public String requestAnimalsByLocation(String location, int radius) throws MalformedURLException {
    String params = String.format("?location=%s&radius=%d&status=adoptable", location, radius);
      ...
      if (null != (animal.get("photos").get(0))) {
        for (JsonNode photo : animal.get("photos")) {
          ...
          if (urlArray.length > 1) {
            sb.append("{\"id\":" + animal.get("id") + ",\"photoId\":" + urlArray[1] + ",\"type\":"
                + animal.get("type") + ",\"fullUrl\":" + "\"" + urlArray[0] + "=" + urlArray[1]
                + "\"" + "}");
            { // logger information
              logger.info("\n{" + "\n\t\"id\":" + animal.get("id") + ",\n\t\"photoId\":"
                  + urlArray[1] + ",\"type\":" + animal.get("type") + ",\n\t\"fullUrl\":" + "\""
